...
namespace ProcessDataTutorial
{
    ...
    public struct OIBSubscriptionData
    {
        ...
        /// <summary>
        /// A SiplacePro line path if a filtered subscription is needed (null otherwise)
        /// </summary>
        public string LineFilter;
        /// <summary>
        /// A SiplacePro station path if a filtered subscription is needed (null otherwise)
        /// </summary>
    ...
    static class OIBSubscriptionHelper
    {
        ...
        /// <summary>
        /// If true then use client authentication
        /// </summary>
        ...
        public static Identifier Subscribe(OIBSubscriptionData data)
        {
            ...
            try
            {
                ...
                {
                    if (subscription.Delivery.NotifyTo.ToEndpointAddress().Uri.AbsoluteUri == data.ServiceUri.AbsoluteUri)
                    {
            ...
            if (subscriptionId == null)    // In case, the subscription does not yet exist: subscribe
            {
                ...
                {
                    xPathFilter = string.Format("LinePath[.=\"{0}\"]", data.LineFilter);
                }
                else if (!string.IsNullOrEmpty(data.StationFilter))
                {
                    xPathFilter = string.Format("StationPath[.=\"{0}\"]", data.StationFilter);
                }
                if (xPathFilter != null)
                {
                    subscribe.Filter = CreateProcessDataFilter(xPathFilter);
                }
        ...
        private static ISubscriptionManager GetSubscriptionManagerProxy(string oibHost, int oibPortHttp)
        {
            ...
            var endpoint= new Uri(string.Format("http://{0}:{1}/ASM.AS.Oib.WS.Eventing.Services/SubscriptionManager/ReliableSecure", oibHost, oibPortHttp));

            // Add endpoint identity to provide information about service identity and open secure communication to service
            EndpointAddress address = new EndpointAddress(endpoint, EndpointIdentity.CreateDnsIdentity("ASM.SW.Service"));
        ...
        private static void AdjustBindingParametersReliable(WSHttpBinding bind)
        {
            if (bind != null)
            {
                ...
                bind.Security.Message.ClientCredentialType = UseClientAuthentication
        ...
        /// <summary>
        /// Create an XPath filter object for the ProcessData subscription
        /// </summary>
        ...
        private static XPathMessageFilter CreateProcessDataFilter(string xpathFilter)
        {
            // Create a name table to be able to construct a XmlNamespaceManager later on
            const string xmlForNameTable = "<ProcessDataRequest  xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\">" +
                                    "<LinePath>LineNamePath</LinePath></ProcessDataRequest>";
            XmlTextReader reader = new XmlTextReader(xmlForNameTable, XmlNodeType.Document, null);
            ...
            // Now create the XmlNamespaceManager from the name table
            ...
            // Create the filter (use "m" namespace since that is the ProcessDataRequest class namespace where the LinePath or StationPath properties belong to)
            return new XPathMessageFilter("//m:" + xpathFilter, nsmgr);                                
        }
        #endregion
    }
}

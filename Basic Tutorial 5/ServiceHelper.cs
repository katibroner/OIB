...
namespace OIB.Tutorial
{
    public class ServiceHelper<T> : IDisposable where T : class
    {
        ...
        /// <param name="useClientAuthentication">Is true use client authentication</param>
        ...
        private void InitializeProxy(Uri mexUri, string transportProtocolName)
        {
            ...
            if (selectedEndpoint != null)
            {
                ...
                // Check if secure endpoint is used
                var endpoint = selectedEndpoint.Address.Uri.ToString();
                if (!selectedEndpoint.Address.Uri.ToString().EndsWith("Secure"))
                {
                    // Make sure to use secure endpoint
                    endpoint += "Secure";
                }

                // Add endpoint identity to provide information about service identity and open secure communication to service
                EndpointAddress address = new EndpointAddress(new Uri(endpoint), EndpointIdentity.CreateDnsIdentity("ASM.SW.Service"));
      ...
      private void AdjustBindingParameters(Binding binding, bool useSecurity)
        {
            ...
            if (bindHttp != null)
            {
                ...
                bindHttp.OpenTimeout = new TimeSpan(0, 0, 0, 10); // 10 seconds
                bindHttp.CloseTimeout = new TimeSpan(0, 1, 0, 0); // 10 seconds
                bindHttp.SendTimeout = new TimeSpan(0, 0, 1, 0); // 1 Minute
            ...
            if (bindTcp != null)
            {
                ...
                bindTcp.OpenTimeout = new TimeSpan(0, 0, 0, 10); // 10 seconds
                bindTcp.CloseTimeout = new TimeSpan(0, 1, 0, 0); // 10 seconds
                bindTcp.SendTimeout = new TimeSpan(0, 0, 1, 0); // 1 Minute
